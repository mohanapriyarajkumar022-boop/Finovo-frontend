...
class ProjectService {
  ...
  async checkServerConnection() {
    try {
      ...
      // Try multiple endpoints including public ones
      const endpointsToTry = ['/health', '/', '/api/health'];
      
      for (const endpoint of endpointsToTry) {
        try {
          // Use the main apiClient for proper headers
          const response = await apiClient.get(endpoint, {
            timeout: 5000,
            validateStatus: function (status) {
              // Consider any response (even 4xx/5xx) as server being reachable
              return status >= 200 && status < 600;
            }
          });
          
          console.log(`✅ Server is reachable at ${endpoint} (Status: ${response.status})`);
          return true;
        } catch (error) {
          // If we get any response (even error), server is running
          if (error.response) {
            console.log(`✅ Server is running (responded with ${error.response.status} for ${endpoint})`);
            return true;
          }
          
          // Network errors mean server might be down
          ...
          if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
            console.log(`⏰ Timeout for ${endpoint}:`, error.message);
            continue;
          }
      ...
      // If all endpoints failed with network errors, server is likely down
      console.log('❌ All connection attempts failed - server is likely down or very slow');
      return false;
  ...
  async getAllProjects() {
    try {
      ...
      // Always try local storage first for faster response
      const localProjects = this.localManager.getAllProjects();
      
      // Then try backend if available
      if (!this.useLocalStorage) {
        try {
          const endpoints = ['/projects', '/project', '/user/projects'];
          ...
          for (const endpoint of endpoints) {
            try {
              ...
              // If we get data from backend, update local storage and return it
              ...
              console.log('📱 Backend error, using local storage:', error.message);
              break;
            }
          }
        } catch (backendError) {
        ...
        try {
          ...
          for (const endpoint of endpoints) {
            try {
              ...
              // For other errors, continue with local storage version
  ...
  handleError(error) {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          return new Error('Project endpoint not found. Using local storage instead.');
        case 500:
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        return new Error('Server is taking too long to respond. Using local storage instead.');
      }
      if (error.code === 'ERR_NETWORK') {
        return new Error('Cannot connect to server. Using local storage instead.');
      }
