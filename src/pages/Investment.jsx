...
const Investment = ({ userSession }) => {
  ...
  const API_BASE = 'http://localhost:5000/api/investment';
  ...
  const fetchRealData = async () => {
    try {
      ...
      const [marketResponse, predictionsResponse, incomeResponse] = await Promise.allSettled([
        ...
        axios.get(`${API_BASE}/user-income`, { headers })
      ]);
  ...
  const handleSaveIncome = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/save-income`, incomeForm, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Tenant-ID': tenantId,
