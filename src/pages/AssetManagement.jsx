...
const MarketDataService = {
  ...
  async getGoldPrice(date = new Date()) {
    // FIX: Ensure date is a Date object before calling toDateString
  ...
  async getCurrentMarketValue(asset) {
    try {
      ...
      switch(asset.type.toLowerCase()) {
        case 'gold':
          // Provide purchaseDate, handled by the fix in getGoldPrice
          marketData = await this.getGoldPrice(asset.purchaseDate);
          // Use appropriate purity rate
...
const AssetManagementSystem = () => {
  ...
  const loadAssets = async () => {
    ...
    try {
      ...
      console.log('No stored assets found, starting fresh:', error);
      setAssets([]);
    }
    setLoading(false);
  };
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <button
            onClick={() => setView('dashboard')}
            className="mb-4 text-blue-600 hover:text-blue-700 font-semibold transition-colors"
          >
            ← Back to Dashboard
          </button>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Enhanced Stats Cards - Updated to Pink with Light Purple Shades */}
